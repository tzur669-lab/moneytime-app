...
function _showTableReport(detailed,filterDates,xlsxOpts){
  ...
  // כמה עבודות נוספות מקסימום בטווח
  var maxExtra=0;
  ...
  allKeys.forEach(function(k){
    ...
    // עמודות עבודות נוספות
    var extras=r.extraJobs||[];
    for(var ei=0;ei<maxExtra;ei++){
      if(extras[ei]){
        var ej=extras[ei];
        var ejr=cpExtra(ej.hours||0,ej.rate||0);
        ...
        rows+='<td style="color:#CBD5E1">-</td><td style="color:#CBD5E1">-</td>';
      }
    }
    // עבודה א סכום / בונוס / סה"כ
  ...
  // --- תחילת תוספת: שורת סיכום לטבלת ה-HTML ---
  rows+='<tr style="background:#1E3A8A;color:white;font-weight:bold;font-size:13px">';
  rows+='<td>סיכום כללי</td><td></td>';
  rows+='<td>'+totalH.toFixed(1)+'</td>'; // סך שעות עבודה
  if(detailed&&ox.rate) rows+='<td></td>';
  if(detailed&&ox.ot) rows+='<td></td><td></td><td></td>';
  for(var ei=0;ei<maxExtra;ei++) rows+='<td></td><td></td>';
  
  rows+='<td>₪'+Math.round(totalBase).toLocaleString('he-IL')+'</td>'; // שכר בסיס שלי
  rows+='<td>₪'+Math.round(totalP-totalBase).toLocaleString('he-IL')+'</td>'; // סך בונוסים
  rows+='<td>₪'+Math.round(totalP).toLocaleString('he-IL')+'</td>'; // סך הכל שכר שלי
  
  if(detailed&&ox.note) rows+='<td></td>';
  if(detailed&&ox.job) rows+='<td></td>';
  if(detailed&&ox.partner) rows+='<td>'+(totalPartner>0 ? '₪'+Math.round(totalPartner).toLocaleString('he-IL') : '')+'</td>'; // סך שכר פרטנר
  rows+='</tr>';

  // שורת שכר משותף (רק אם יש פרטנר בטווח התאריכים)
  ...
  for(var ei2=0;ei2<maxExtra;ei2++){
    var lbl=HEL[ei2+1]||String(ei2+2);
    headerCols+='<th>\u05e9\u05e2\u05d5\u05ea \u05e2\u05d1 '+lbl+'</th><th>\u05e1\u05db\u05d5\u05dd '+lbl+' \u20aa</th>';
  }
  headerCols+='<th>\u05e2\u05d1 \u05d0 \u20aa</th>';
  headerCols+='<th>\u05d1\u05d5\u05e0\u05d5\u05e1 \u20aa</th>';
  headerCols+='<th>\u05e1\u05d4"\u05db \u20aa</th>';
  ...
  allKeys.forEach(function(k){
    ...
    for(var ei4=0;ei4<maxExtra;ei4++){
      if(extras2[ei4]){var ej2=extras2[ei4];var ejr2=cpExtra(ej2.hours||0,ej2.rate||0);cols.push(ej2.hours);cols.push(Math.round(ejr2.base));}
      else{cols.push('');cols.push('');}
    }
    cols.push(Math.round(pr.base));
    cols.push(bon.toFixed(0));
    cols.push(Math.round(dayTot.total));
  ...
  sumCols.push(Math.round(totalBase)); // שכר שלי
  sumCols.push(Math.round(totalP-totalBase)); // בונוסים
  sumCols.push(Math.round(totalP)); // סך הכל שלי
  
  if(detailed&&ox.note) sumCols.push('');
  if(detailed&&ox.job) sumCols.push('');
  if(detailed&&ox.partner) sumCols.push(totalPartner>0 ? Math.round(totalPartner) : ''); // שכר פרטנר
  
  csvLines.push(sumCols.map(function(cv){return'"'+String(cv).replace(/"/g,'""')+'"';}).join(','));
  
  // שורה לשכר המשותף באקסל (אם יש פרטנר)
  if(detailed&&ox.partner&&totalPartner>0){
  ...
  var htmlContent='<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8">'
    ...
    +'body{font-family:Arial,sans-serif;font-size:13px;color:#0F172A;direction:rtl;padding:12px;overflow-x:auto}'
  ...
  titleEl.style.cssText='font-size:14px;font-weight:400;flex:1';
  ...
  reader.onload=function(ev){
    ...
    try{
      if(ext==='json'){
        ...
        _restoreFromCSV(text);
      } else {
        _restoreFromJSON(text);
      }
    }catch(err){
      setRestoreStatus('שגיאה: '+err.message, true);
    }
  };
...
function _restoreFromJSON(text){
  ...
  catch(e){ setRestoreStatus('קובץ JSON לא תקין: '+e.message,true); return; }
  ...
  if(Array.isArray(p)){
    p.forEach(function(item){
      ...
      if(hours<=0)return;
      existing[item.date]=Object.assign({},existing[item.date]||{},{
        hours:hours,
        rate:parseFloat(item.rate)||defaultRate,
        note:item.note||''
      });
      restoredDays++;
    });
    ...
    if(dateKeys.length>0){
      dateKeys.forEach(function(k){
        existing[k]=Object.assign({},existing[k]||{},p[k]);
        restoredDays++;
      });
    } else {
      setRestoreStatus('פורמט JSON לא מוכר',true);
      return;
    }
  }

  if(restoredDays===0 && !restoredSettings){
    setRestoreStatus('הקובץ ריק או לא מכיל נתוני שעות',true);
    return;
  }
  D.s(existing);
  var msg='✅ שוחזרו '+restoredDays+' ימים'+(restoredSettings?' + הגדרות':'');
...
function _restoreFromCSV(text){
  ...
  var iDate =findCol('date','תאריך');
  var iHours=findCol('hours','שעות');
  var iRate =findCol('rate','תעריף');
  var iNote =findCol('note','הערה','notes');

  if(iDate===-1) iDate=0;
  if(iHours===-1) iHours=1;
  ...
  if(count===0){
    setRestoreStatus('לא נמצאו נתונים. ודא שיש עמודות Date ו-Hours',true);
    return;
  }
  D.s(existing);
...
async function restoreFromDrive(){
  if(!aToken){
    alert('חבר תחילה את חשבון Google Drive');
    return;
  }
  ...
  try{
    await loadDr();
    setRestoreStatus('✅ שוחזר מ-Google Drive בהצלחה!');
    setTimeout(function(){location.reload();},1000);
  }catch(e){
    setRestoreStatus('שגיאה: '+e.message,true);
  }
}

function execDel(){if(!confirm('האם אתה בטוח?'))return;const r=document.getElementById('delR').value;if(r==='all'){D.s({});location.reload();return;}let data=D.g();getDts(r,'delS','delE').forEach(d=>delete data[fk(d)]);D.s(data);closeM('mDel');renderCal();updRep();saveDr();vib();}
